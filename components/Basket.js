...
export default class Basket extends React.Component {
  ...
  validVoucher = async () => {
    try {
      await axios.post('http://localhost:3001/vouchers', { voucher: this.state.voucher });
  ...
  render() {
    return (
      <Modal
        title={
          <span>
            My basket - <span id='basketItemNumber'>{this.state.basket.productsCount}</span>
          </span>
        }
        ...
        footer={[
          ...
          <Button
            ...
            disabled={!this.state.basket.productsCount ? true : false}
          >
            Buy now { this.state.basket.totalWithVoucher ? (
              <React.Fragment>
                £{this.state.basket.totalWithVoucher} <span className='oldPrice'>£{this.state.basket.total}</span>
              </React.Fragment>
      ...
      >
        <React.Fragment>
          <p style={{ fontWeight: 'bold', textAlign: 'center' }}>* The vouchers are not combinable</p>
          <div>
            {this.state.basket.products.map(item => {
              const product = this.state.products.find(product => product.id === item.id);
                    ...
                    <div>
                      <div>
                        <img
                          width='50'
                          height='50'
                          src={`${item.cover}/${PRODUCT_COVER_WIDTH}/${PRODUCT_COVER_HEIGHT}`}
                        />
                      </div>
                      <div>
                        <p>£{item.price}</p>
                        ...
                        <div>
                          ...
                          />
                          <Button onClick={() => this.removeProduct(item)}>Remove product</Button>
                        </div>
                      </div>
                    </div>
                    <div>
