...
const computeTotalPrice = (products) => products.reduce((acc, curr) => ({
  totalPrice: acc.totalPrice + curr.totalPrice
}));

const computeProductsCount = (products) => products.reduce((acc, curr) => ({
...
const applyVoucher = (db, total, voucher = null) => {
  ...
  const footerWearProduct = basket.products.find((product) => {
    return [CATEGORY_WOMEN_FOOTWEAR, CATEGORY_MEN_FOOTWEAR].includes(product.category) && product.quantity > 0;
  });
...
.then(db => {
  ...
  app.post('/basket', (req, res) => {
    ...
    .find({ id: req.body.product.id })

    const getProductsBasket = db.get('basket.products')
    const alreadyInBasketRequest = getProductsBasket
    .find({ id: req.body.product.id })
    ...
    if (req.body.type === ADD_ONE_IN_BASKET) {
      if (alreadyInBasket) {
        ...
        product
        .assign({ quantity: product.value().quantity - 1 })
        .write()

        alreadyInBasketRequest
        .assign({ quantity, totalPrice })
        .write()
      } else {
        product
        .assign({ quantity: product.value().quantity - 1 })
        ...
        getProductsBasket
        .push({ ...req.body.product, quantity: 1, totalPrice: parseFloat(req.body.product.price, 10) })
        .write()
      }
    }
    ...
    if (req.body.type === REMOVE_PRODUCT) {
      if (alreadyInBasket) {
        product
        .assign({ quantity: product.value().quantity + alreadyInBasket.quantity })
        .write()
      }
    ...
    if (db.get('basket.products').value().length) {
      ...
      productsCount = computeProductsCount(db.get('basket.products').value()).quantity;
